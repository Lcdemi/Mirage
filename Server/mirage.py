...
from variables import MirageConfig
...
def fwd_pwnboard(target, result):
    ...
    headers = {'Content-Type': 'application/json', 'Authorization': PWNBOARD_AUTH_TOKEN}
...
def fwd_discord(target, response):
    ...
    # Send and check result
    try:
        result = requests.post(DISCORD_WEBHOOK_URL, json = data, timeout = 5)
...
def main():
    ...
    # Load configuration using dataclass
    config = MirageConfig.load()
    
    # Set global variables from config for backwards compatibility
    global ALL_HOSTS, ALL_CA, ALL_DC, ALL_ICMP, ALL_IIS, ALL_FTP
    global ALL_MSSQL, ALL_RDP, ALL_SMB, ALL_SSH, ALL_WINRM
    global PORT, TIMEOUT, CONCURRENCY, THROTTLE_MS
    global PWNBOARD_URL, PWNBOARD_AUTH_TOKEN, DISCORD_WEBHOOK_URL
    ...
    TIMEOUT = config.other.TIMEOUT
    CONCURRENCY = config.other.CONCURRENCY
    THROTTLE_MS = config.other.THROTTLE_MS
    
    PWNBOARD_URL = config.logging.PWNBOARD_URL
    PWNBOARD_AUTH_TOKEN = config.logging.PWNBOARD_AUTH_TOKEN
    DISCORD_WEBHOOK_URL = config.logging.DISCORD_WEBHOOK_URL
